...
def generate_char_image(generator, char):
    ...
    # 🔥 MAX CONTRAST THRESHOLDING 🔥
    gray = fake_img.convert("L")
    thresholded = gray.point(lambda p: 0 if p < 180 else 255)

    # Apply brightness tweak AFTER threshold
    brightness_factor = random.uniform(*S.brightness_range)
    thresholded = ImageEnhance.Brightness(thresholded).enhance(brightness_factor)
